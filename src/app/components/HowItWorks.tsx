...
export default function HowItWorks() {
  return (
    <section id="come-funziona" className="mx-auto max-w-md px-5 py-12 lg:max-w-6xl lg:py-20">
      <h2 className="text-center text-3xl font-extrabold lg:text-4xl">Come funziona</h2>
      ...
      <ol className="mt-8 space-y-6 lg:mt-14 lg:grid lg:grid-cols-5 lg:gap-4 lg:space-y-0">
        {steps.map(({ n, title, text, Illo }, i) => (
          <li key={n} className="relative">
            <div className="flex h-full items-center gap-4 rounded-card bg-white p-4 shadow-soft lg:flex-col lg:items-center lg:gap-2 lg:p-5 lg:text-center">
              <div className="shrink-0">
                <Illo className="h-24 w-24 lg:h-28 lg:w-28" />
              </div>
              <div>
                <div className="flex items-center gap-2 lg:flex-col lg:gap-1.5">
                  ...
                  <h3 className="text-lg font-extrabold lg:text-base">{title}</h3>
                </div>
                <p className="mt-1 text-sm text-dotto-ink/70">{text}</p>
              </div>
            </div>
            {i < steps.length - 1 && (
              <div className="ml-8 h-6 w-0.5 bg-dotto-ink/10 lg:hidden" aria-hidden />
            )}
          </li>
        ))}
      </ol>
